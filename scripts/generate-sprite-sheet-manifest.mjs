...
const GENERATED_COLOR_IDS_PATH = "src/game/core/generated/unitColorIds.js";
const SUPPORTED_ANIMATION_IDS = ["idle", "walk", "attack"];
const MOVEMENT_PHASE_IDS = ["start", "loop", "end"];
const BLANK_ANIMATION_FRAME = "blank";
const PREFERRED_UNIT_COLOR_ORDER = ["purple", "blue", "green", "orange"];
...
async function discoverUnitColorIds(root) {
  const colorRoot = path.resolve(root, "assets/sprites/units");
  const entries = await fs.readdir(colorRoot, { withFileTypes: true });

  return entries
    .filter((entry) => entry.isDirectory() && /^[a-z0-9][a-z0-9_-]*$/i.test(entry.name))
    .map((entry) => entry.name.toLowerCase())
    .sort((left, right) => {
      const leftIndex = PREFERRED_UNIT_COLOR_ORDER.indexOf(left);
      const rightIndex = PREFERRED_UNIT_COLOR_ORDER.indexOf(right);
...
export async function generateUnitSpriteSheetManifest({ root = process.cwd() } = {}) {
  ...
  const unitColorIds = await discoverUnitColorIds(root);

  for (const colorId of unitColorIds) {
    colorCoverage[colorId] = [];
  }
  ...
  for (const unitTypeId of unitTypeIds) {
    ...
    for (const colorId of unitColorIds) {
      ...
      const hasStaticSprite = await pathExists(staticPath);
      ...
      colorCoverage[colorId].push(hasStaticSprite || Boolean(sheetSpec));
    }
  }

  const colorAvailability = Object.fromEntries(
    unitColorIds.map((colorId) => [colorId, colorCoverage[colorId].every(Boolean)])
  );
  ...
  const colorIdsPath = path.resolve(root, GENERATED_COLOR_IDS_PATH);
  const colorIdsOutput = [
    "// This file is generated by scripts/generate-sprite-sheet-manifest.mjs.",
    "// Do not edit by hand.",
    "",
    `export const GENERATED_UNIT_COLOR_IDS = ${JSON.stringify(unitColorIds, null, 2)};`,
    ""
  ].join("\n");

  await fs.mkdir(path.dirname(colorIdsPath), { recursive: true });
  await fs.writeFile(colorIdsPath, colorIdsOutput, "utf8");
